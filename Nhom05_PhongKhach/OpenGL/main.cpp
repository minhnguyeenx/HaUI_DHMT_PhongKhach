...
{
	TaoVatLieuVer2(BLACK, NHUA);
	VeHinhLapPhuong(Scale(0.4, 0.1, 0.4), mat4());
}
...
{
	TaoVatLieuVer2(BLACK, NHUA);
	VeHinhLapPhuong(Scale(0.3, 0.2, 0.2), Translate(0, 0, 0));
}

void quat_cay_truc_moto()
{
	TaoVatLieuVer2(RED, NHUA);
	VeHinhLapPhuong(Scale(0.03, 0.02, 0.02), Translate(-0.16, 0, 0));
}

void quat_cay_num_chan()
{
	TaoVatLieuVer2(BLACK, NHUA);
	VeHinhLapPhuong(Scale(0.03, 0.06, 0.06), Translate(-0.19, 0, 0));
}

void canh_quat(GLfloat alpha)
{
	TaoVatLieuVer2(WHITE, NHUA);
	VeHinhLapPhuong(Scale(0.01, 0.3, 0.06), RotateX(alpha) * RotateY(10) * Translate(-0.19, 0.18, -0.03));
}
...
void bo_canh()
{
	quat_cay_truc_moto();
	quat_cay_num_chan();
	//model = model * Translate(0, 0, 0);
	canh_quat(0);
	canh_quat(90);
	canh_quat(180);
	canh_quat(270);
}
void quat_cay(GLfloat x, GLfloat y, GLfloat z)
{
	model = model * Translate(x, y, z);
	quat_cay_de();
	...
	quat_cay_truc();
	//xoay ngang
	model = model * Translate(0.04, 0.65, 0) * RotateY(quat_cay_goc_ngang) * RotateZ(quat_cay_goc_cao);
	quat_cay_moto();
	//quay
	model = model * RotateX(quat_cay_quay);
	bo_canh();
}
...
void quat_tran_moto()
{
	...
	VeHinhLapPhuong(Scale(0.1, 0.1, 0.1), Translate(0, 0.04, 0));
}
void quat_tran_canh_quat(GLfloat alpha)
{
	TaoVatLieuVer2(GRAY, NHUA);
	VeHinhLapPhuong(Scale(0.9, 0.01, 0.2), RotateY(alpha) * RotateX(5) * Translate(0.5, 0, 0));
}
...
void quat_tran(GLfloat x, GLfloat y, GLfloat z)
{
	model = model * Translate(x, y, z);
	quat_tran_truc();
	model = model * Translate(0, -0.2, 0) * RotateY(-quat_tran_quay);
	quat_tran_bo_canh();
}
...
void chanban1() {
	...
	VeHinhLapPhuong(Scale(0.05, 0.4, 0.05), Translate(-0.375, -0.205, 0));
	VeHinhLapPhuong(Scale(0.05, 0.4, 0.05), Translate(0.375, -0.205, 0));
	VeHinhLapPhuong(Scale(0.75, 0.05, 0.05), Translate(0.0, -0.155, 0));
}
void chanban2() {
	TaoVatLieuVer2(BROWN, NHUA);
	VeHinhLapPhuong(Scale(0.05, 0.4, 0.05), Translate(-0.375, -0.205, -0));
...
void ban(GLfloat x, GLfloat y, GLfloat z) {
	...
	model = model * Translate(0, 0, 0.775) * RotateX(chan_ban1_quay);
	chanban1();

	model = modelchanban2;
	model = model * Translate(0, 0, -0.775) * RotateX(chan_ban2_quay);
	chanban2();
}
